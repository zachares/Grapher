...
flags.DEFINE_string('output_dir', '', 'Path to output directory to store the'
                    ' paired dataset.')
flags.DEFINE_string('text_dir', '', 'Directory that contains Wikipedia Text')


def pair_graphs_with_wikitext(subset: str, graph_dir: str, output_dir: str, text_dir: str):
  ...
  wikitext_articles = list(wikitext.RawDataset(subset=subset, version='raw', data_dir=text_dir))
...
def main(_):
  for subset in ['train', 'valid', 'test']:
    pair_graphs_with_wikitext(subset, FLAGS.freebase_dir, FLAGS.output_dir, FLAGS.text_dir)
